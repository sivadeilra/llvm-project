...
void CodeViewDebug::emitHotPatchInformation() {
  ...
  for (const auto &F : MMI->getModule()->functions()) {
    if (!F.isDeclarationForLinker() && F.hasFnAttribute(Attribute::MarkedForWindowsHotPatching)) {
      auto *SP = F.getSubprogram();
      StringRef SymbolName = SP->getLinkageName();
      if (SymbolName.empty()) {
      ...
      OS.AddComment("Name");
      auto LinkageName = SP->getLinkageName();
...
void CodeViewDebug::emitDebugInfoForJumpTables(const FunctionInfo &FI) {
  ...
  for (const auto& JumpTable : FI.JumpTables) {
    ...
    // auto theTable = llvm::dyn_cast<>(JumpTable.Table);
    JumpTable.Table->dump();
